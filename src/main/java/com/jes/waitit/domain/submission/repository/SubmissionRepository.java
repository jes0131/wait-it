...
public interface SubmissionRepository extends JpaRepository<Submission, Long> {
    @Query(value = "SELECT s.waiting_num FROM submissions s WHERE s.reservation_id = :reservationId AND s.submission_state IN ('CANCELED', 'RESOLVED') AND s.is_deleted = false ORDER BY s.updated_at DESC LIMIT 1", nativeQuery = true)
    ...
    List<Submission> findAllByReservationIdAndIsDeletedFalse(Long reservationId);
}
